...
/////////////////////////////////////////////////////////////////////////////
// Read plain NVM without fixed calibration matrix K and do not return feature indices
...
// Read NVM with fixed calibration matrix K
bool LoadNVM_FiexedK(ifstream& in, vector<CameraT>& camera_data, vector<Point3D>& point_data,
	vector<Point2D>& measurements, vector<int>& ptidx, vector<int>& camidx,
	vector<string>& names, vector<int>& ptc)
{
	int rotation_parameter_num = 4;
	...
	// For fixed calibration in VisualSFM
	string fixedK;
	in >> fixedK;
	float fx, cx, fy, cy, dum;
	in >> fx >> cx >> fy >> cy >> dum;
...
// Read NVM with fixed calibration matrix K and return the indices for the features
bool LoadNVM_FiexedK_FeatIdx(ifstream& in, vector<CameraT>& camera_data, vector<Point3D>& point_data,
	vector<Point2D>& measurements, vector<int>& ptidx, vector<int>& camidx, vector<int>& featidx,
	vector<string>& names, vector<int>& ptc)
{
	int rotation_parameter_num = 4;
	bool format_r9t = false;
	...
	for (int i = 0; i < npoint; ++i)
	{
		...
		for (int j = 0; j < npj; ++j)
		{
			...
			camidx.push_back(cidx);    //camera index
			featidx.push_back(fidx);   //feature index
			ptidx.push_back(i);        //point index
...
// Read NVM and return the indices for the projected features
bool LoadNVM_FeatIdx(ifstream& in, vector<CameraT>& camera_data, vector<Point3D>& point_data,
	vector<Point2D>& measurements, vector<int>& ptidx, vector<int>& camidx, vector<int>& featidx,
	vector<string>& names, vector<int>& ptc)
{
	int rotation_parameter_num = 4;
...
void SaveNVM_Featidx(const char* filename, vector<CameraT>& camera_data, vector<Point3D>& point_data,
	vector<Point2D>& measurements, vector<int>& ptidx, vector<int>& camidx, vector<int>& featidx,
	vector<string>& names, vector<int>& ptc)
{
	if (featidx.size() != measurements.size()) { cout << "Feat # != Measurement # ..." << endl; system("pause"); }
	std::cout << "Saving model to " << filename << "...\n";
	ofstream out(filename);
	...
	for (size_t i = 0, j = 0; i < point_data.size(); ++i)
	{
		...
		for (; j < je; ++j)    out << camidx[j] << ' ' << featidx[j] << ' ' << measurements[j].x << ' ' << measurements[j].y << ' ';
...
bool RemoveInvisiblePoints_withFeat(vector<CameraT>& camera_data, vector<Point3D>& point_data,
	vector<int>& ptidx, vector<int>& camidx, vector<int>& featidx,
	vector<Point2D>& measurements, vector<string>& names, vector<int>& ptc)
{
	vector<float> zz(ptidx.size());
	for (size_t i = 0; i < ptidx.size(); ++i)
